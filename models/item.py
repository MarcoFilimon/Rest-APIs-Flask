...
class ItemModel(db.Model):
    __tablename__ = "items"  # create / use a table named items

    id = db.Column(db.Integer, primary_key=True) # primary key is automatically indexed by SQLAlchemy (starts from 1)
    name = db.Column(db.String(80), nullable=False) # nullable=False means that the field cannot be null in the database.
    ...
    # foreign key is a reference to the primary key of another table (in this case "items" table)
